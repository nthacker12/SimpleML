...
# digit(0-9) class.  Suck as [0.14, 0.8, 0, 0, 0, 0, 0, 0, 0, 0.06]
...
# create an interactive session that can span multple code blocks.  Don't
# forget to explicitly close the session with sess.close()
sess = tf.Session()

# perform the initilization which is only the initialization of all global variables
sess.run(init)

# Perform 1000 training steps
for i in range(1000):
    batch_xs, batch_ys = mnist.train.next_batch(100)    # get 100 random data points for the data.  batch_xs = image, batch_ys = digit(0-9) class
